...
import { useFilterRows } from '@/composables/useFilterRows'
    ...
    const previousWeekDate = computed(() => getDateWithOffset(-7));
    ...
    const doSearch = async (offset) => {
      ...
      try {
         const queryParams = new URLSearchParams(
                {dateFrom: previousWeekDate.value, dateTo: currentDate.value, page: pageNo, key: key, limit: limit.value }
            );
            ...
            const labels = table.rows.map(item => item.supplier_article)
            const chartValues = table.rows.map(item => item.quantity);
    ...
    // Вспомогательная функция для перевода строки в Camel Case
